...
export default function AchievementManage() {
  ...
  const openCreate = () => {
    setEditData(null)
    setForm({ projectId: '', title: '', subtitle: '', paragraph1: '', media_urls: [], conclusion: '', writeDate: new Date().toISOString().slice(0, 10) })
    setShowModal(true)
  }
  const openEdit = (a) => {
    setEditData(a)
        ...
        <tbody>
          {list.map(a => (
            <tr key={a.id}>
              ...
              <td>{a.write_date?.slice(0, 10)}</td>
              <td>
                <button className="btn-sm" onClick={() => openEdit(a)}>编辑</button>
                <button className="btn-sm btn-danger" onClick={() => handleDelete(a)}>删除</button>
              </td>
            </tr>
          ))}
          {list.length === 0 && (
            <tr>
              <td colSpan="5" className="text-center">暂无成果</td>
            </tr>
          )}
        </tbody>
      </table>
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              ...
              <div className="form-group">
                <label>正文内容</label>
                <textarea value={form.paragraph1} onChange={e => setForm(f => ({ ...f, paragraph1: e.target.value }))} rows={4} />
              </div>
              <div className="form-group">
                <label>结束语</label>
                <textarea value={form.conclusion} onChange={e => setForm(f => ({ ...f, conclusion: e.target.value }))} rows={2} />
              </div>
              <div className="form-group">
                <label>撰稿日期 *</label>
                <input type="date" value={form.writeDate} onChange={e => setForm(f => ({ ...f, writeDate: e.target.value }))} required />
              </div>
              <div className="form-group">
                <label>媒体素材</label>
                ...
                <button type="button" className="btn-secondary" onClick={() => setShowPicker(true)}>+ 从素材库选择</button>
                {showPicker && <MediaPicker onSelect={(url) => { if (url) addMedia(url); setShowPicker(false) }} />}
              </div>
